...
import {RapydURL, contentColor} from '../../env';
...
class Setup extends Component {
  ...
  async startComponent(){
    ...
    this.mount = true;
      if (this.context.value.kyc) {
        ...
        // Polka Address
        try {
          const session = await AsyncStorage.getItem('polkaAddress');
          if (session !== null) {
            this.context.setValue({
              polkaAccount: JSON.parse(session).value,
            });
            this.checkAddress = true;
          } else {
            this.checkAddress = false;
        ...
        if (this.checkWallet && this.checkPincode && this.checkAddress) {
          ...
          this.props.navigation.navigate('CheckPin'); // CheckPin
        } else {
          this.setState({
            stage: 1,
          });
  ...
  async storePolkaAddress(value) {
    try {
      await AsyncStorage.setItem('polkaAddress', JSON.stringify({value}));
      this.context.setValue({
        account: value,
      });
    } catch (e) {
                  ...
                  this.setState(
                    ...
                    () =>
                      setTimeout(async () => {
                        ...
                        const polka = keyring.createFromUri(
                          wallet.mnemonic.phrase,
                        );
                        await this.storeUserPrivs({
                          address: wallet.address,
                          polkaAddress: polka.address,
                          phrase: wallet.mnemonic.phrase,
                          privateKey: wallet.privateKey,
                          polkaKey: JSON.stringify(polka.toJson()),
                        });
                        await this.storeUserAddress(wallet.address);
                        await this.storePolkaAddress(polka.address);
                        this.setState({
                          phrase: wallet.mnemonic.phrase,
                          text: 'Fill KYC',
                        });
