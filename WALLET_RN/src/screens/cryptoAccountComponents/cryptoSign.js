        ...
        if (session !== null) {
          if (val === JSON.parse(session).value) {
            try {
              ...
              if (session !== null) {
                const wallet = new ethers.Wallet( JSON.parse(session).value.privateKey, this.provider )
                const signedTx = await wallet.signTransaction(this.props.transaction);
              ...
              if (session !== null) {
                ...
                this.web3.eth.accounts
                  .signTransaction(
                    ...
                    JSON.parse(session).value.privateKey,
                  )
                  .then(res => this.props.signTrans(res))
