...
import { NODE_ENV_NETWORK_RCP, NODE_ENV_API_APIKEY, contentColor, headerColor, RapydURL } from "../../../env"
    ...
    async acceptAndSign() {
        ...
        this.axios({
            ...
            .then((response) => {
                this.axios({
                    method: 'get',
                    url: `${RapydURL}/transaction-decide`,
                    headers: {
            ...
            <View style={GlobalStyles.container} >
                ...
                }
                {
                    ...
                    <View style={{ position: "absolute", top: 18, right: 18 }}>
                        <IotReciever publish={this.state.publish} sub_topics={[`/PolkaSend/WalletConnect/${this.context.value.account}`]} callback={this.callBackIoT} callbackPublish={() => this.mount && this.setState({ publish: { message: '', topic: '', } })} />
                    </View>
                }
                {
                    this.state.stage === 0 &&
