...
class IotReciever extends Component {
    ...
    async componentDidMount() {
        ...
        let AWSConfiguration = require('./aws-configuration.js');
        ...
        await new Promise((resolve) => {
            AWS.config.credentials.get((err, data) => {
                if (!err) {
                    let params = { IdentityId: AWS.config.credentials.identityId };
    ...
    render() {
        return (
            <View>
                <Icon name="dot-circle-o" size={24} color={this.state.connect ? headerColor : "red"} />
            </View>
        );
    }
}
